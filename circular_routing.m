...
nofrings= sinkx/k;
fprintf('No. of rings=%d', nofrings);
...
%               fprintf('%dth :(%d, %d) ', SN(j).id, R(count).nodeloc(1), R(count).nodeloc(2));
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%finding neighbor cells/grids  of each cell in each ring
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for i=1:nofrings
   fprintf('\n for ring %d : ', i);     
maxn=8; 
...
%%%%% cyclic /clockwise routing of packets from the primary nodein the basic ring 
cr_ring=SN(src).rno;% cr stores the ring number where cyclic routing has to take place
ct_cr=1;    %to increment the counter  during clockwise routing
Rd(round).Cyclic(1).nodeid=pn;
Rd(round).Cyclic(1).cell=pncn;
Rd(round).Cyclic(1).ring=pnrn;
 
if pncn~=R(cr_ring).totalcells %to check if phantom node in basic ring is not in the last cell
 for cr=pncn+1:R(cr_ring).totalcells
     if R(cr_ring).C(cr).nodes>0
         next_pn=R(cr_ring).C(cr).chead;
         ...
         Rd(round).Cyclic(ct_cr).nodeid=pn;
         Rd(round).Cyclic(ct_cr).cell=cr;
         Rd(round).Cyclic(ct_cr).ring=cr_ring;
     end
 end
 if cr==R(cr_ring).totalcells
     for crr=1:pncn
         next_pn=R(cr_ring).C(crr).chead;
         cordinates=[SN(pn).x, SN(pn).y; SN(next_pn).x,SN(next_pn).y]; %stores the two cordinates we have to compute the distance
         d=pdist(cordinates,'euclidean');
         if d<d0 
             et= Eelec*b+ Efs*b*(d^2);
         ...
         SN(pn).ct_pkt=SN(pn).ct_pkt+1;
          Rd(round).Cyclic(ct_cr).nodeid=pn;
         Rd(round).Cyclic(ct_cr).cell=crr;
         Rd(round).Cyclic(ct_cr).ring=cr_ring;
     
     end 
 end
 else
     for cr=1:R(cr_ring).totalcells
         next_pn=R(cr_ring).C(cr).chead;
         cordinates=[SN(pn).x, SN(pn).y; SN(next_pn).x,SN(next_pn).y]; %stores the two cordinates we have to compute the distance
         d=pdist(cordinates,'euclidean');
         if d<d0 
 ...
 pncn=randi([1,R(pnrn).totalcells]);%randomly select any cell from the event ring to transmit through spr
 ct_spr=0;
while(pnrn>1)
    nbrs=0;
    for nn=1:maxn
...
%%%%%%%%%%%recording hop count  for cyclic routing and shortest path routing %%%%%%%%%%%%%%%%%%%%%%
Rd(round).hop_cr=ct_cr;
Rd(round).hop_spr=ct_spr;
%%%%%%%%%%recording total hops traversed by each packet%%%%%%%%%%%%%
total_hops=ct_cr+ct_spr;
...
for pc=1:total_hops %%to assign all the nodes in a single structure, hops+1 as first node is the src node and counter for spr starts from 1
   if (pc<ct_cr)|| (pc==ct_cr)
       Rd(round).Allnodes(pc).pn_nid=Rd(round).Cyclic(pc).nodeid;
       Rd(round).Allnodes(pc).ring=Rd(round).Cyclic(pc).ring;
       Rd(round).Allnodes(pc).cell=Rd(round).Cyclic(pc).cell;
   elseif (pc<ct_cr+ct_spr)|| (pc==ct_cr+ct_spr)
       Rd(round).Allnodes(pc).pn_nid=Rd(round).SPR(i).nodeid;
       Rd(round).Allnodes(pc).ring=Rd(round).SPR(i).ring;
       Rd(round).Allnodes(pc).cell=Rd(round).SPR(i).cell;
       i=i+1;
   end
end
...
if round==1
   ...
   for s=total_hops:-1:2
       if (Rd(round).Allnodes(s).ring==adrn) & (Rd(round).Allnodes(s).cell==adcn)
           ctst=ctst+1;
           opt(ctst).id=ctst;
           opt(ctst).pn_nid=Rd(round).Allnodes(s-1).pn_nid;
           opt(ctst).ring=Rd(round).Allnodes(s-1).ring;
           opt(ctst).cell=Rd(round).Allnodes(s-1).cell;
       end
   end
end
if round~=1 & ctst>0
    choice=randi([1,ctst],1,1);
    adv=opt(choice).pn_nid;
    adrn=opt(choice).ring;
    adcn=opt(choice).cell;
    ctbtk=ctbtk+1;
    adversarylocation(ctbtk).node=adv;
    adversarylocation(ctbtk).ring=adrn;
    adversarylocation(ctbtk).cell=adcn;
