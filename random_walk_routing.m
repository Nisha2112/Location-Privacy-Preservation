...
maxn=8; 
      for j=1: R(i).totalcells
       ncno=0;
%      fprintf('\n for cell %d the neighbor nodes are: ', R(i).C(j).cno);
      for nn=1: maxn
              if (nn==1)
...
ct_rw=1;% counter starts from 1 so that the source node is counted as the first hop
hr=10;
Rd(round).RW(1).nodeid=pn;
Rd(round).RW(1).cell=pncn;
Rd(round).RW(1).ring=pnrn; 
hr=hr-1;%source node reduces the hop count by one
while (hr>0) %till the primary node does not reach to 1st ring of the hop for random walk gets 0
    nbrs=0;
    for nn=1:maxn
        if (R(pnrn).C(pncn).NC(nn).nodes>0)
            nbrs=nbrs+1;
            Neighborlist(nbrs).id=ct_rw;
...
next_pn=Neighborlist(pn_random).pn;
...
ct_rw=ct_rw+1;
hr=hr-1; %to reduce the hop count by 1 after each hop movement
SN(pn).ct_pkt=SN(pn).ct_pkt+1;
Rd(round).RW(ct_rw).nodeid=pn;
Rd(round).RW(ct_rw).cell=pncn;
Rd(round).RW(ct_rw).ring=SN(pn).rno; 
end
    

%%%%shortest path routing to deliver the packet to sink %%
ct_spr=0;
while(pnrn>1)
    nbrs=0;
    for nn=1:maxn
...
%%%%%%%%%%%recording hop count  for random routing and shortest path routing %%%%%%%%%%%%%%%%%%%%%%
Rd(round).hop_rw=ct_rw;
Rd(round).hop_spr=ct_spr;
%%%%%%%%%%recording total hops traversed by each packet%%%%%%%%%%%%%
total_hops=ct_rw+ct_spr;
...
for pc=1:total_hops %%to assign all the nodes in a single structure, hops+1 as first node is the src node and counter for spr starts from 1
   if (pc<ct_rw)|| (pc==ct_rw)
       Rd(round).Allnodes(pc).pn_nid=Rd(round).RW(pc).nodeid;
       Rd(round).Allnodes(pc).ring=Rd(round).RW(pc).ring;
       Rd(round).Allnodes(pc).cell=Rd(round).RW(pc).cell;
   elseif (pc<ct_rw+ct_spr)|| (pc==ct_rw+ct_spr)
